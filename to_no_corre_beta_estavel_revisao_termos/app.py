...
8. Contato
Durante os testes, dúvidas e relatos devem ser enviados pelo mesmo canal em que o aplicativo foi fornecido. Antes da distribuição pública aberta, deverá ser informado um canal permanente no arquivo PRIVACIDADE.md."""
...
def fetch_linkedin(p):
    ...
    if internship_search_mode(p)=="somente" and state_search:
        local_query=", ".join((state_search,"Brazil"))
    for q in terms:
...
class App(tk.Tk):
    ...
    def ui(self):
        ...
        def resize_detail_panel(event):
            available=max(80,event.width-32)
