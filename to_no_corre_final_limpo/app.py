...
def default_profile():
    ...
    return {
        ...
        "aceitar_remoto":True,"mostrar_compativeis_fora_regiao":False,
...
def adapt_profile_to_cv(p,text):
    ...
    if "Jurídico" in prof["areas"]:
        queries += ["assistente jurídico","auxiliar jurídico","controladoria jurídica","paralegal","legal operations","estágio direito","estágio jurídico"]
    if "Tecnologia / TI" in prof["areas"]:
        queries += ["suporte n1","help desk","service desk","assistente de suporte","estágio TI","estágio ADS","estágio suporte técnico"]
    ...
    for course in prof["courses"][:2]:
        queries += [f"estágio {course}",f"assistente {course}",f"analista júnior {course}"]
    if not queries:
...
def location_decision(job,p):
    ...
    local_allowed=any(city and re.search(rf"\b{re.escape(city)}\b",loc) for city in allowed)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.db();self.load_feedback_profile();self.migrate_v19();self.migrate_v23();self.migrate_v24();self.migrate_v25();self.migrate_v26();self.ui();self.refresh()
    ...
    def ui(self):
        ...
        for text,value in [("Todas","todas"),("Home office","home_office"),("Estágio","estagio")]:
    ...
    def edit_profile(self):
        w,created=self.managed_window("configuracoes","Configurações","620x650",modal=True)
        if not created:return
        ...
        ttk.Entry(locf,textvariable=cities).pack(fill="x",pady=(3,7))
        ...
        navf=ttk.LabelFrame(body,text="Navegador para candidaturas",padding=10);navf.pack(fill="x",pady=(0,10))
        ...
        agef=ttk.Frame(body);agef.pack(fill="x",pady=(2,10))
        ...
        def save():
            ...
            if vals:self.p["cidades_presencial"]=vals;self.p["cidades_presencial_hibrido"]=vals
            ...
            save_json_file(PROFILE_PATH,self.p);messagebox.showinfo("Configurações","Alterações salvas.",parent=w);w._managed_close()
        actions=ttk.Frame(body);actions.pack(fill="x",pady=8)
        ttk.Button(actions,text="Atualizar descrições pendentes",command=lambda:self.update_pending_descriptions()).pack(side="left")
        ttk.Button(actions,text="Reavaliar vagas",command=lambda:self.recalculate_with_notice(w)).pack(side="left",padx=6)
        ttk.Button(actions,text="Salvar",command=save).pack(side="right")
